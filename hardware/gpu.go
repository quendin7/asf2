...
func getGPUDetailsFromSysfs(cardIndex int) (GPUDetails, error) {
	...
	vIDBytes, errV := os.ReadFile(vendorPath)
	dIDBytes, errD := os.ReadFile(devicePath)
	...
	if subVBytes, err := os.ReadFile(subsystemVendorPath); err == nil {
		subV := strings.TrimSpace(strings.TrimPrefix(string(subVBytes), "0x"))
		if subDBytes, err := os.ReadFile(subsystemDevicePath); err == nil {
...
func getGPUInfoWindows() string {
	out, err := exec.Command("wmic", "PATH", "Win32_VideoController", "GET", "Name", "/format:value").Output()
	if err != nil {
		return "N/A"
	}
