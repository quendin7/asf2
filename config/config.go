...
func GetUserConfigDir() string {
	configPathOnce.Do(func() {
		if path := os.Getenv("ASF_CONFIG_DIR"); path != "" {
			configDirPath = path
			return
		}
		...
		if err == nil {
			...
			configDirPath = filepath.Join(xdgConfigHome, "asf")
		} else {
			configDirPath = "./.asf_config"
		}
	})
...
func GetConfigFilePath() string {
	if path := os.Getenv("ASF_CONFIG"); path != "" {
		return path
	}
...
func EnsureConfigAndArtExist(configFilePath string) error {
	...
	if _, err := os.Stat(defaultLogoPath); os.IsNotExist(err) {
		...
		if err := os.WriteFile(defaultLogoPath, []byte(artContent), 0644); err != nil {
